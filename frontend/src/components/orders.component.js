...
const Orders = () => {
	...
	function changeWaiterForOrder(){
			...
			var url = "http://localhost:8080/waiter/update/" + changeWaiter.id
			axios.put(url, {
	...
	function changeSingleOrder(){
			...
			var url = "http://localhost:8080/order/update/" + changeOrder.id
	...
	function addSingleOrder(){
			...
			var url = "http://localhost:8080/order/create"
			axios.post(url, {
	...
	function deleteSingleOrder(id){
			var url = "http://localhost:8080/order/delete/" + id
			axios.delete(url, {
			}).then(response => {
					if(response.status == 200){
							setRefreshData(true)
